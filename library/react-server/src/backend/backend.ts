...
export class ResponseInfo {
  ...
  llm: string | LLM; // The name of the LLM queried (the nickname in Aggrag)
...
/**
 * Queries RAG(s) with root prompt template `prompt` and prompt input file.
 * Soft-fails if API calls fail, and collects the errors in `errors` property of the return object.
 * 
 * @param id a unique ID to refer to this information. Used when cache'ing responses. 
 * @param rag a string, list of strings, or list of RAG spec dicts specifying the RAG(s) to query.
 * @param prompt the prompt template, with any {{}} vars
 * @param vars a dict of the template variables to fill the prompt template with, by name. 
               For each var, can be single values or a list; in the latter, all permutations are passed. (Pass empty dict if no vars.)
 * @param progress_listener (optional) a callback whenever an RAG response is collected, on the current progress
 * @param cancel_id (optional) the id that would appear in CancelTracker if the user cancels the querying (NOT the same as 'id' --must be unique!)
 * @returns a dictionary in format `{responses: RAGResponse[], errors: string[]}`
 */
export async function queryRAG(
  id: string,
  rag: string | (string | LLMSpec)[],
  prompt: string,
  vars: Dict,
  chat_histories?: ChatHistoryInfo[] | { [key: string]: ChatHistoryInfo[] },
  no_cache?: boolean,
  ...
  if (Array.isArray(rag) && rag.length === 0)
    throw new Error(
      "POST data rag is improper format (not string or list, or of length 0).",
    );

  // Ensure rag param is an array
  ...
  const rag_to_cache_filename: Dict<string> = {};
  const past_cache_files: Dict<string | LLMSpec> = {};
  if (typeof cache === "object" && cache.cache_files !== undefined) {
    const past_cache_files: Dict = cache.cache_files;
    const past_cache_filenames: Array<string> = Object.keys(past_cache_files);
    rags.forEach((rag_spec: string | LLMSpec) => {
      ...
      for (const [filename, cache_rag_spec] of Object.entries(
        ...
        if (matching_settings(cache_rag_spec, rag_spec)) {
          rag_to_cache_filename[extract_llm_key(rag_spec)] = filename;
          found_cache = true;
          break;
        }
      }
      if (!found_cache) {
        ...
        rag_to_cache_filename[extract_llm_key(rag_spec)] = new_filename;
        cache.cache_files[new_filename] = rag_spec;
        past_cache_filenames.push(new_filename);
      }
    });
  } else {
    ...
    cache = { cache_files: {}, responses_last_run: [] };
    const prev_filenames: Array<string> = [];
    rags.forEach((rag_spec: string | Dict) => {
      const fname = gen_unique_cache_filename(id, prev_filenames);
      rag_to_cache_filename[extract_llm_key(rag_spec)] = fname;
      cache.cache_files[fname] = rag_spec;
      prev_filenames.push(fname);
    });
  }
  ...
  // For each RAG, generate and cache responses:
  const responses: { [key: string]: Array<RawLLMResponseObject> } = {};
  const all_errors: Dict<string[]> = {};
  async function query(rag_spec: string | Dict): Promise<any> {
    // Get RAG model name and any params
    const rag_str = extract_llm_name(rag_spec);
    const rag_nickname = extract_llm_nickname(rag_spec);
    const rag_params = extract_llm_params(rag_spec);
    const rag_key = extract_llm_key(rag_spec);
    const _vars = vars;

    const chat_hists = (
      chat_histories !== undefined && !Array.isArray(chat_histories)
        ? chat_histories[rag_nickname]
        : chat_histories
    ) as ChatHistoryInfo[];

    // Create an object to query the RAG, passing a storage key for cache'ing responses
    const cache_filepath = rag_to_cache_filename[rag_key];
    const prompter = new PromptPipeline(
      prompt,
      no_cache ? undefined : cache_filepath,
    );

    // Prompt the RAG with all permutations of the input prompt template:
    // NOTE: If the responses are already cache'd, this just loads them (no RAG is queried, saving $$$)
    const resps: Array<RawLLMResponseObject> = [];
    const errors: Array<string> = [];
    let num_resps = 0;
    let num_errors = 0;
    ...
    try {
      console.log(`Querying ${rag_str}...`);

      // Yield responses for 'rag' for each prompt generated from the root template 'prompt' and template variables in 'properties':
      for await (const response of prompter.gen_rag_responses(
        _vars,
        rag_str,
        rag_params,
        chat_hists,
        ...
        if (response instanceof LLMResponseError) {
          // The request failed
          console.error(
            `error when fetching response from ${rag_str}: ${response}`,
          );
          num_errors += 1;
        ...
        // Update the current 'progress' for this rag.
        // (this implicitly triggers any callbacks defined in the Proxy)
        progressProxy[rag_key] = {
          success: num_resps,
          error: num_errors,
        };
      }
      ...
      if (e instanceof UserForcedPrematureExit) {
        ...
        console.error(
          `Error generating responses for ${rag_str}: ${(e as Error).message}`,
        );
        throw e;
      }
    }
  ...
  // Request responses simultaneously across RAGs
  const tasks: Array<Promise<any>> = rags.map(query);
  ...
  // Reorder the responses to match the original vars dict ordering of keys and values
  const vars_lookup: { [key: string]: Dict } = {}; // we create a lookup table for faster sort
  Object.entries(vars.query).forEach(([varname, vals]: any) => {
    vars_lookup[varname] = {};
    vals.forEach((vobj: Dict | string, i: number) => {
      const v = typeof vobj === "string" ? vobj : vobj?.text;
      vars_lookup[varname][v] = i;
  ...
  const cache_filenames = past_cache_files;
  rags.forEach((rag_spec: string | LLMSpec) => {
    const filename = rag_to_cache_filename[extract_llm_key(rag_spec)];
    cache_filenames[filename] = rag_spec;
  });
...
export async function caching_responses(
  response: LLMResponse[],
  past_cache_files: Dict<string | LLMSpec>,
  id: string,
) {
  StorageCache.store(`${id}.json`, {
    cache_files: past_cache_files,
    responses_last_run: response,
  });
}
