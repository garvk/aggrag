...
// from aggrag.promptengine.models import LLM
...
export async function upload_to_flask_backend(
  route: string,
  data: FormData,
): Promise<Dict> {
  return fetch(`${FLASK_BASE_URL}app/${route}`, {
...
/**
 * Tries to determine if the Aggrag front-end is running on user's local machine (and hence has access to Flask backend).
...
export async function call_anthropic(
  ...
  while (responses.length < n) {
    ...
    if (APP_IS_RUNNING_LOCALLY()) {
      ...
      // We're on the aggrag.ai server; route API call through a proxy on the server, since Anthropic has CORS policy on their API:
...
export async function call_google_palm(
  ...
  // --depending on if it's the Python or Node JS API. Aggrag needs a consistent name, so we must convert snake to camel:
  const casemap = {
...
export async function call_dalai(
  ...
  if (APP_IS_RUNNING_LOCALLY()) {
    ...
    throw new Error(
      "Cannot call Dalai: The Aggrag app does not appear to be running locally. You can only call Dalai-hosted models if" +
        "you are running a server with the Dalai Node.js package and have installed Aggrag on your local machine.",
    );
  }
}
...
async function call_custom_provider(
  ...
  if (!APP_IS_RUNNING_LOCALLY())
    throw new Error(
      "The Aggrag app does not appear to be running locally. You can only call custom model providers if you are running Aggrag on your local machine, from a Flask app.",
    );
...
function _extract_rag_responses(response: Array<Dict>): Array<LLMResponseData> {
  return response.map((r: any) => {
    ...
    if (typeof r.response === "string") {
      return (
        r.response +
        `
        TIME TAKEN: ${r.time_taken.total} `
      );
    } else {
      console.log(r.response, typeof r.response);
      return JSON.stringify({
        response: r.response,
        time_taken: r.time_taken.total,
      });
    }
  });
}
/**
 * Given a RAG and a response object from its API, extract the
 * text response(s) part of the response object.
 */
export function extract_rag_responses(
  response: Array<string | Dict> | Dict,
): Array<LLMResponseData> {
  return _extract_rag_responses(response as Array<Dict>);
}
...
export async function upload_raw_docs_file(params: FormData): Promise<Dict> {
  const response = await upload_to_flask_backend("uploadFile", params);

  // Fail if an error is encountered
  if (response === undefined || !response.success) {
    throw new Error(`Unable to save file: ${JSON.stringify(response)}`);
  }
