...
/** All color palettes in Aggrag. Import to use elsewhere. */
...
export const initRAGProviderMenu: LLMSpec[] = [
  {
    ...
    description: (
      <>
        Nothing fancy, foundational RAG that generates context-aware responses{" "}
        <br />
        by retrieving relevant information from a given document set. Leverages{" "}
        <br />
        llama-index; using SimpleDirectoryReader for document loading and <br />
        VectorStoreIndex for index creation.
        <br />
        <br />
        Streaming and chat history is allowed, but currently disabled.
        <br />
        <br />
        <i>
          Pro tip: answers general questions from the document which are
          directly <br />
          referenced. Little to no insight is provided more than the source
          material.
        </i>
      </>
    ),
  },
  {
    ...
    description: (
      <>
        More power to user queries. Breaks down a user query into smaller <br />
        relevant queries. Each query generates context-aware responses, and{" "}
        <br />
        the responses are combined into a single comprehensive answer. <br />
        <br />
        Streaming is not possible. Chat history is possible, but currently
        <br />
        disabled.
        <br />
        <br />
        <i>
          Pro tip: provides a wider description of concepts/facts provided in
          the
          <br />
          document, even beyond the scope of the document but limited to the
          <br />
          context.
        </i>
      </>
  ...
  },
  {
    ...
    description: (
      <>
        Consolidates info context into clear summaries.Adapts a bottom-up <br />
        approach by clustering information chunks to form a hierarchical tree{" "}
        <br />
        structure.
        <br />
        <br />
        ...
        <br />
        <i>
          Pro tip: Best at answering factual questions which requires more
          <br />
          specific prompts. Prefer this when numbers or values are expected.
          <br />
        </i>
      </>
    ),
...
export interface StoreHandles {
  ...
  // The RAG providers available in the drop-down list
  AvailableRAGs: LLMSpec[];
  setAvailableRAGs: (specs: LLMSpec[]) => void;
  ...
  getImmediateInputNode: (targetHandles: string[], node_id: string) => any[];
...
// across Aggrag and ReactFlow components.
const useStore = create<StoreHandles>((set, get) => ({
  nodes: [],
  edges: [],

  // Available LLMs in Aggrag, in the format expected by LLMListItems.
  AvailableLLMs: [...initLLMProviders],
  setAvailableLLMs: (llmProviderList) => {
    set({ AvailableLLMs: llmProviderList });
  },

  // Available RAGs in Aggrag, in the format expected by RAGListItems.
  AvailableRAGs: [...initRAGProviderMenu],
  setAvailableRAGs: (ragProviderList) => {
    set({ AvailableRAGs: ragProviderList });
  },
  ...
  getImmediateInputNode: (_targetHandles, node_id) => {
    const getNode = get().getNode;
    const edges = get().edges;
    const inputNode: any[] = [];
    edges.forEach((e) => {
      if (
        ...
        const src_node = getNode(e.source);
        if (src_node && src_node.type !== undefined) inputNode.push(src_node);
      }
    });
    return inputNode;
  },
